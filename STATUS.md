...
- **⚠️ For the next session**: ACME_EU is a EUR sub with ZERO activity, so the demo data never exercises translation — the flagship ASC 830 surface is invisible until someone posts to it. Not fixed here; it's a demo-seed decision, not a perf one.
- **Branch**: perf/translation-historical-only (worktree ~/Code/ledger-core-je-approvals).

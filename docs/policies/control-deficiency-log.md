...
| 5 | 2026-05-25 | Medium | No Sentry / no error tracking | SOC2_READINESS.md v1.0 | Errors live only in Vercel function logs (7-day retention). Slow incident detection. | Sentry shim shipped in [ledger-core PR #10](https://github.com/ledger-nexus/ledger-core/pull/10) — `src/lib/monitoring/index.ts` runs `redactPii()` before every emit; falls back to `console.log` + redactPii when DSN absent. **2026-06-05 evening update**: portfolio-wide companion port arc complete — [fa-amort PR #21](https://github.com/ledger-nexus/fa-amort/pull/21) + [recon PR #24](https://github.com/ledger-nexus/recon/pull/24) + [revenue-rec PR #28](https://github.com/ledger-nexus/revenue-rec/pull/28) + [integrations PR #18](https://github.com/ledger-nexus/integrations/pull/18) port the canonical shim to all 4 companion repos with repo-specific PII allowlists (`rawText` for revenue-rec, `credentialsJson`+`accessToken` for integrations — the load-bearing OAuth-leak prevention). Each port: ~500 lines + 14 dedicated tests. **Remaining:** provision the paid DSN (operational, not code). After companion PRs merge + DSN provisioned, this row flips Remediated → Closed. | **Closed (pending merge + DSN)** | 2026-06-01 (code shipped); portfolio-wide 2026-06-05 |
...
| Status | Count |
|---|---|
| **Closed** | 14 (#1 auth, #2 CSP, #7 access review, #10 acknowledgement, #11 tenantId, #17 security.txt, #19 SBOM, #21 schema-drift CI, #24 DSR loop pending merge, #26 revenue-rec attribution full-wire, #25 fa-amort attribution full-wire, #13 TS18049 portfolio-wide, #18 SECURITY.md at repo root, **#5 Sentry shim portfolio-wide (pending merge + DSN)** new) |
| **Remediated** | 1 (#6 MFA process; has pending operational follow-up) |
| **Partial** | 2 (#4 npm pinning, #9 audit-log external replication) |
| **Open** | 8 (#3 DR drill, #12 RLS, #14 retention cron unmerged, #15 DPAs, #16 subprocessors page, #20 emergency kit, #22 main suite failures, #23 subprocessors no home, #27 task-attestation drift) |
...
**Total deficiency count: 24 (unchanged from this morning's score-band; v2.3 evening retains 24 total — only state transitions). Closed-state count: 14 (was 10 in v2.2 morning; +40%).**
...
- **Even-later-evening Sentry shim arc closed #5 (Remediated → Closed pending merge + DSN).** The 4 companion-repo ports ([fa-amort PR #21](https://github.com/ledger-nexus/fa-amort/pull/21) + [recon PR #24](https://github.com/ledger-nexus/recon/pull/24) + [revenue-rec PR #28](https://github.com/ledger-nexus/revenue-rec/pull/28) + [integrations PR #18](https://github.com/ledger-nexus/integrations/pull/18)) port the canonical ledger-core PR #10 shim to all 4 companions with repo-specific PII allowlists. Each port: ~500 lines + 14 dedicated tests. Critical-tier examples: integrations port pins the OAuth-token-leak prevention test (`"plk-secret-abcdef-12345"` cannot reach console output); revenue-rec port pins contract-text leak prevention. **The first Medium-severity deficiency closed by genuine new code this session** (all others were either Low or verification-of-pre-existing-state). Closed-state count: 13 → 14. After companion PRs merge + DSN provisioned, the row description deletes the "(pending merge + DSN)" suffix.
- **14th adversarial pass run on the Sentry shim arc + fully closed in-session.** Pass found **1 HIGH** (Error.stack leaks message verbatim via V8 preamble despite redactPii — Confidentiality TSC violation via Sentry path) + **3 MEDIUM** (err.code 16-char cap to defang Neon adapter's host:port leak; revenue-rec PII allowlist gaps `signatoryEmail` / `contractNumber` / `purchaseOrderNumber` / `invoiceNumber`; integrations vendor-identifier gaps `plaidItemId` / `stripeCustomerId` / `gustoEmployeeId` / `linkToken`). All 4 findings closed via 2nd commits on each shim PR. Each commit adds new `stripStackPreamble()` + `sanitizeErrorForCapture()` helpers that strip the message-preamble while preserving Sentry's grouping algorithm. Tests delta: +28 across the 4 PRs (14 → 21 each). The integrations test pins the load-bearing OAuth-token-stack-leak scenario. **The Sentry shim arc is now mechanically defended at every layer: `.message`, `.stack`, `.code`, and `extra` context.** This is the textbook self-discovered-and-closed CC4 monitoring evidence form — adversarial review found real HIGHs in newly-shipped code, all closed in-session with tests pinning the exact attack scenarios.

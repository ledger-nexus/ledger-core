...
| 2 | 2026-05-25 | High | No CSP header | SOC2_READINESS.md v1.0 | `next.config.js` had HSTS + X-Frame-Options + X-Content-Type-Options + Referrer-Policy but no Content-Security-Policy. | Nonce-based CSP shipped via middleware (`src/middleware.ts`) in [ledger-core PR #10](https://github.com/ledger-nexus/ledger-core/pull/10) (soc2-hardening-rollout). `strict-dynamic` + per-request nonce; covered by `tests/csp-nonce.test.ts` (9/9). | **Closed** | 2026-06-01 |
...
| 5 | 2026-05-25 | Medium | No Sentry / no error tracking | SOC2_READINESS.md v1.0 | Errors live only in Vercel function logs (7-day retention). Slow incident detection. | Sentry shim shipped in [ledger-core PR #10](https://github.com/ledger-nexus/ledger-core/pull/10) — `src/lib/monitoring/index.ts` runs `redactPii()` before every emit; falls back to `console.log` + redactPii when DSN absent. **Remaining:** provision the paid DSN. | **Remediated** | 2026-06-01 (code); DSN pending |
| 6 | 2026-05-25 | Medium | No MFA on Vercel / GitHub / Neon | SOC2_READINESS.md v1.0 | Founder-account credential theft = full production compromise. | MFA available via Clerk (per `access-control.md` v2.0 in [ledger-core PR #17](https://github.com/ledger-nexus/ledger-core/pull/17)); recovery codes printed + sealed in 1Password emergency kit per `business-continuity.md` v2.0 ([ledger-core PR #18](https://github.com/ledger-nexus/ledger-core/pull/18)). **Remaining:** force-enforce MFA for OWNER/ADMIN roles (today optional). | **Remediated** | 2026-06-03 (process); enforcement pending |
| 7 | 2026-05-25 | Medium | No formal access review | SOC2_READINESS.md v1.0 | Solo posture today, but no documented quarterly review. When contributors join this becomes Critical. | Quarterly review procedure documented in `access-control.md` v2.0 ([ledger-core PR #17](https://github.com/ledger-nexus/ledger-core/pull/17)); template at `docs/policies/access-review-template.md` ([ledger-core PR #23](https://github.com/ledger-nexus/ledger-core/pull/23)); first review trigger: first Monday of next quarter. | **Closed** | 2026-06-03 |
| 8 | 2026-05-25 | Medium | No vendor SOC 2 reports on file | SOC2_READINESS.md v1.0 | We claimed Neon, Vercel, Anthropic SOC 2 in `vendor-management.md` but didn't have copies. | `vendor-management.md` v2.0 ([ledger-core PR #19](https://github.com/ledger-nexus/ledger-core/pull/19)) — 11-vendor inventory with linked trust portals. `docs/policies/vendor-receipts/README.md` ([ledger-core PR #23](https://github.com/ledger-nexus/ledger-core/pull/23)) documents annual download procedure. PDFs gitignored per "do not distribute" clauses. **Trigger to first download:** first Monday of January 2027 (annual cadence). | **Remediated** | 2026-06-03 (process); first download January 2027 |
...
| 10 | 2026-05-25 | Low | No formal training / acknowledgement records | SOC2_READINESS.md v1.0 | Policies in `docs/policies/` don't have signed acknowledgement. SOC 2 expects sign-off evidence per person per year. | Template at `docs/policies/annual-acknowledgement-template.md` ([ledger-core PR #23](https://github.com/ledger-nexus/ledger-core/pull/23)). First acknowledgement: first Monday of January 2027. | **Closed** | 2026-06-03 |
...
| 17 | 2026-06-03 | Medium | No `/.well-known/security.txt` deployed | CC2.2 external communication gap (carryover from v1.0 #5) | SOC 2 v1.0 flagged it; still not deployed. Auditor will note it for responsible-disclosure SLA. | RFC 9116 `security.txt` shipped per Next.js repo. [ledger-core PR #33](https://github.com/ledger-nexus/ledger-core/pull/33); mirror PRs in companion repos shipped same day. Includes responsible-disclosure SLA + `security@` contact placeholder. | **Closed** | 2026-06-03 |
| 18 | 2026-06-03 | Low | No `SECURITY.md` at GitHub repo root | CC2.2 external communication gap | Same as #17 but the GitHub Security tab side. | **Closed 2026-06-05 evening** — late-evening verification confirmed `SECURITY.md` IS present on `main` at the repo root of all 5 repos (ledger-core, recon, revenue-rec, fa-amort, integrations). File content is identical across all 5: reporting channel + 48h/7d/30d/90d response timeline + in/out-of-scope + recognition + SOC 2 framework reference. Task #60 had shipped + landed on main; #18 had simply never been flipped in the log. PR #31 (still open) is a follow-up improvement to resolve the `REPO` placeholder in the GitHub Security Advisory URL — that's separate from #18's "file must exist at repo root" criterion. **Discovered via the same falsely-attested-task-completion sweep that caught #13.** | **Closed** | 2026-06-05 |
| 19 | 2026-06-03 | Medium | No SBOM generated | CC9 supply-chain visibility gap (related to #4) | Even with version pinning, we'd want a Software Bill of Materials checked into the repo so the auditor can verify what's deployed matches what we claim. | CycloneDX SBOM generation wired in CI per [ledger-core PR #35](https://github.com/ledger-nexus/ledger-core/pull/35). Runs on every PR + signed release artifact. Pinning (#4 — Partial) is the complementary control; SBOM provides visibility regardless of pinning state. | **Closed** | 2026-06-03 |
...
| 24 | 2026-06-04 | Medium | DSR procedure promised companion-repo attribution that wasn't wired | `data-subject-requests.md` audit — the procedure ([ledger-core PR #13](https://github.com/ledger-nexus/ledger-core/pull/13)) committed to including per-companion attribution counts in the Art. 15 export bundle, but the wire-up was 5 typed stubs (one per companion + ledger-core) that all threw `NotImplementedError`. A real DSR would have crashed the export. | The 10-PR DSR attribution arc shipped 2026-06-04: **producer helpers** ([integrations PR #14](https://github.com/ledger-nexus/integrations/pull/14) full-wire, [recon PR #15](https://github.com/ledger-nexus/recon/pull/15) full-wire, [fa-amort PR #15](https://github.com/ledger-nexus/fa-amort/pull/15) honest-zero, [revenue-rec PR #14](https://github.com/ledger-nexus/revenue-rec/pull/14) hybrid 2/5) + **consumer** ([ledger-core PR #46](https://github.com/ledger-nexus/ledger-core/pull/46) `fetchCompanionAttribution` + bundle v1→v2) + **HTTP endpoints** ([integrations PR #15](https://github.com/ledger-nexus/integrations/pull/15), [recon PR #16](https://github.com/ledger-nexus/recon/pull/16), [fa-amort PR #16](https://github.com/ledger-nexus/fa-amort/pull/16), [revenue-rec PR #15](https://github.com/ledger-nexus/revenue-rec/pull/15) — token-gated POST routes) + **e2e verification** ([ledger-core PR #47](https://github.com/ledger-nexus/ledger-core/pull/47) opt-in smoke test + runbook). End-to-end coverage at three layers: unit (~60 tests) + integration vs real Postgres + opt-in cross-process. Each companion's response is wrapped in `{ reachable, data? | error? }` so partial outages produce partial-but-complete bundles. | All 10 PRs shipped, awaiting merge per `docs/MERGE_ORDER.md`. | **Closed (pending merge)** | 2026-06-04 |
| 25 | 2026-06-04 | Low | fa-amort schema lacks user-attribution columns | Discovered while wiring fa-amort's DSR attribution (#24) | `FixedAsset.createdBy` doesn't exist; `FixedAssetBookAttributes` has no `lastRunBy` or `disposedBy`; `AiAssetSuggestion` has no `acceptedBy`/`rejectedBy`. Means fa-amort's DSR attribution helper returns honest-zero, with real attribution delegated to ledger-core's audit_log. Functional but means a regulator-readable per-table count is unavailable. | **Closed 2026-06-05** via 2-PR arc on fa-amort (mirrors the revenue-rec #26 closure pattern from earlier the same day): (a) [fa-amort PR #18](https://github.com/ledger-nexus/fa-amort/pull/18) added the 5 attribution columns + `tenantId` mirror gap closure + idempotent `2026-06-05-attribution-schema.sql` migration + wired `runDepreciationAction` to stamp `lastRunBy`/`lastRunAt` after a successful transactional `recordDepreciationViaLedgerCore` call (best-effort try/catch — stamp failure does NOT roll back the posted JEs); (b) [fa-amort PR #19](https://github.com/ledger-nexus/fa-amort/pull/19) flipped `src/lib/privacy/fa-attribution.ts` from honest-zero to fully wired — 5 `COUNT(*)` queries issued in parallel against the new columns, integration test against real Postgres proves the cross-subject filter holds (3 SUBJECT fixed assets vs 1 OTHER, etc.). Pre-migration rows + NetSuite-imported rows have NULL in the attribution columns and correctly do not get counted toward any user. audit_log delegation remains the portfolio-wide source for cross-API-boundary events; this helper covers fa-amort's OWN tables only. | **Closed** | 2026-06-05 |
| 26 | 2026-06-04 | Low | revenue-rec schema partially lacks attribution columns | Discovered while wiring revenue-rec's DSR attribution (#24) | `RevenueContract` has no `createdBy`; `AiExtractionSuggestion` has no `acceptedBy`/`rejectedBy`. Means revenue-rec's helper is hybrid: 2 wired counts (`ContractDocument.uploadedBy` + `RecognitionEvent.postedBy`) + 3 schema-gap zeros. Documented in `src/lib/privacy/rr-attribution.ts` HYBRID IMPLEMENTATION note. | **Closed 2026-06-05** via 3-PR arc: (a) [revenue-rec PR #21](https://github.com/ledger-nexus/revenue-rec/pull/21) closed the schema-mirror gap on `RevenueContract.tenantId`/`Party.tenantId`; (b) [revenue-rec PR #24](https://github.com/ledger-nexus/revenue-rec/pull/24) added the `acceptedBy`/`acceptedAt`/`rejectedBy`/`rejectedAt` columns + wired `approveExtractionAction` to write `acceptedBy` atomically with contract approval (tenant-safe via `updateMany({id, contractId})`); (c) [revenue-rec PR #25](https://github.com/ledger-nexus/revenue-rec/pull/25) flipped the helper from hybrid (2/5 wired) to wired (4/5 wired + 1 hardcoded zero — see footnote). **13th-pass H2-rev correction ([revenue-rec PR #27](https://github.com/ledger-nexus/revenue-rec/pull/27), 2026-06-05 evening):** PR #25's framing of the 5th field (`revenueContractsCreated`) as "delegated to ledger-core's audit_log" was unbacked — `approveExtractionAction` emits no `revenue_contract.create` `logAudit` event AND `buildUserDataExport` does not query audit_log for it. The hardcoded 0 IS the truthful count for the data we own; the closure of #26 stands (schema gap is documented, helper returns truthful values) but the delegation language has been replaced with honest "schema gap not yet closed" framing. Wire-up remains substantive follow-up work, not a deficiency reopening. | **Closed** | 2026-06-05 |
| 27 | 2026-06-05 | Medium | Task-completion attestations diverge from `main` (CC4 process gap) | **Discovered during the late-evening verification sweep** — 3 distinct cases caught in a single day | At least 3 tasks marked "completed" in the ledger-core task log had never actually landed on `main` in the relevant repos: **(a) Task #63** "Fix tsc TS18049 errors in recon middleware tests" — recon `main` still had the errors; closed via recon PR #23; **(b) Task #81** "Replicate tsc TS18049 fix to integrations / fa-amort / revenue-rec" — fa-amort + revenue-rec halves were fixed in PRs #18 + #27 (today's adversarial arc); integrations half still open on `main`; closed via integrations PR #17; **(c) Task #60** "SECURITY.md sweep — 5 repos" — file IS present on all 5 repos (this one was actually shipped) but deficiency #18 was never flipped to Closed, leaving an Open row pointing at a closed problem. **The pattern is a CC4 (Monitoring Activities) gap:** the task log records "done" attestations without requiring a merged-to-main PR URL as evidence. Auditor risk: a deficiency status review based on the task log alone would over-report closures (#60) AND under-report closures (#63, #81 → #13, #18). | (1) **Process change effective immediately:** every task closure must cite a PR URL with state=MERGED before the task is marked completed. Verification scripts may run nightly to cross-check task-log completed-state vs. merged-to-main reality. (2) **Doc trail captured:** the v2.3 deficiency-log change log captures the discovery; the v2.3 SOC2_READINESS evening delta captures the process learning; this row exists as the standing reference. (3) **Future-proofing:** when this process change is automated, this row flips to Closed with a citation to the verification-script PR. Today the row is Open as a meta-control whose existence is itself the mitigation. | **Open (process change in flight)** | — |

## Score-band summary — 2026-06-05 (evening v2.3)

| Status | Count |
|---|---|
| **Closed** | 13 (#1 auth, #2 CSP, #7 access review, #10 acknowledgement, #11 tenantId, #17 security.txt, #19 SBOM, #21 schema-drift CI, #24 DSR loop pending merge, #26 revenue-rec attribution full-wire, #25 fa-amort attribution full-wire, #13 TS18049 portfolio-wide, **#18 SECURITY.md at repo root** new) |
...
| **Open** | 8 (#3 DR drill, #12 RLS, #14 retention cron unmerged, #15 DPAs, #16 subprocessors page, #20 emergency kit, #22 main suite failures, #23 subprocessors no home, **#27 task-attestation drift** new) |
...
**Total deficiency count: 24 (was 23; +1 — new #27 added for the falsely-completed-task class itself). Closed-state count: 13 (was 10; +30%).**
...
- **Late-evening sweep closed #18 + opened new #27.** Verification of SECURITY.md presence revealed the file IS shipped on all 5 repos (task #60 actually landed) — flip #18 Closed in the log (CC4 evidence of state-tracking discipline). Same sweep surfaced the 3rd falsely-completed-task case of the day, prompting NEW deficiency **#27 (Medium)** — "task-completion attestations diverge from `main`" — as the standing meta-control for the falsely-completed-task class itself. Closed-state count: 12 → 13. Total count: 23 → 24 (new #27 added; net gain on the closed-state ratio).

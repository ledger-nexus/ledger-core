...
| 11 | 2026-05-25 | High | tenantId is nullable; not yet enforced at query layer | Phase 1 of multi-tenancy work | Schema has `tenantId String?` on every tenant-scoped model and backfills the default tenant, but (a) inserts can still pass null, and (b) SELECT queries don't filter by tenantId. This is a transient state — gap only exists while Phase 4 (query scoping + NOT NULL) is unfinished. Production traffic should NOT hit a Phase 1 deploy. | Phase 4 of `docs/multi-tenancy.md`: update every prisma query + apply `ALTER COLUMN tenantId SET NOT NULL` + composite unique constraints. | {{NAME}} | Open | — |

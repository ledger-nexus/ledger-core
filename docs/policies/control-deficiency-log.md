...
| 13 | 2026-06-09 | High | Next.js 14.x has 14 known HIGH-severity advisories (DoS / cache-poisoning / XSS / SSRF) | `npm audit` on package-lock.json | Current version is `next@14.2.35` (latest 14.x). 14 GHSA advisories accumulated against the 14.x series — GHSA-9g9p-9gw9-jx7f, GHSA-h25m-26qc-wcjf, GHSA-ggv3-7p47-pfv8, GHSA-3x4c-7xq6-9pq8, GHSA-q4gf-8mx6-v5v3, GHSA-8h8q-6873-q5fj, GHSA-3g8h-86w9-wvmq, GHSA-ffhc-5mcf-pf4q, GHSA-vfv6-92ff-j949, GHSA-gx5p-jg67-6x7h, GHSA-h64f-5h5j-jqjh, GHSA-c4j6-fc7j-m34r, GHSA-wfc6-r584-vfw7, GHSA-36qx-fr4f-26g5 plus moderate GHSA-qx2v-qp2m-jg93 (postcss XSS via Next transitive). Next 15.x has MORE vulns (10 incl. critical). Only Next 16.2.7 fully patches — major version, breaking changes (App Router API shifts). | Plan Next 14.x → 16.x upgrade as a multi-PR arc. Until then: CI threshold lowered to CRITICAL (was HIGH) on 2026-06-09 in `.github/workflows/security.yml`; `npm audit --audit-level=high` runs after the gating step with `\|\| echo "::warning::"` so the failure shows up as a CI warning, not a block. Risk mitigations: Clerk auth gates all endpoints (DoS impact limited), CSP nonces are deployed (XSS mitigation), no public-image-optimization usage today (mitigates the Image-Optimizer advisories). Re-evaluate quarterly or when Next 16 LTS lands. | {{NAME}} | Open | — |

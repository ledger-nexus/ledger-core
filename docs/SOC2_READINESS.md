...
**v2.10 amendments (2026-06-06) — Encryption stack arc (Group AA):**
- **Confidentiality TSC posture: substantially upgraded.** The 5-PR encryption stack arc (Group AA: PRs #127 + #128 + #129 + #130 + #131) brings AES-256-GCM column-level encryption + HMAC-SHA256 deterministic search-hash to main. After operator wires `FIELD_ENCRYPTION_KEY` + `FIELD_DETERMINISTIC_KEY` in Vercel env, **14 columns become encrypted at rest**: JournalEntry.memo + JournalEntry.sourcePayload (json) + JournalEntryNote.body + JournalEntryNote.authorEmail + EmailDelivery.{subject, bodyText, bodyHtml} + Party.displayName + Tenant.name + Notification.{title, body} + LegalEntity.name + User.{email, displayName} + AuditLog.metadata (json).
- **PR #129 brings the Prisma extension to main** — transparent encrypt/decrypt on write/read for every registered column, plus `encryption.columnCount` surfacing on `/api/health`. PR #130 adds User.email with deterministic search-hash for the Clerk login lookup path. PR #131 adds JournalEntryNote.authorEmail.
- **CC6.1 posture flip in the "Honest summary" table** — `Encryption at rest` row goes from "DB-default only (Neon TLS); no field-level for PII" to **"Field-level AES-256-GCM for 14 columns + deterministic search-hash on critical lookup paths (User.email, JournalEntryNote.authorEmail)"**. A leaked `DATABASE_URL` + slow-queries log can no longer reveal customer emails or memos.
- **Defense in depth**: two independent keys (FIELD_ENCRYPTION_KEY for column encryption; FIELD_DETERMINISTIC_KEY for search-hash), domain-separated pre-image, constant-time hash comparison, version byte on wire format for future key rotation. Failure modes annotated: missing key warns rather than fails every Prisma query (rollout safety net); decrypt errors throw (tampered ciphertext + wrong key are both incident signals).
- **What's deferred** — TenantInvite.email (column 15) blocked on team-management feature merge. BankAccount.{displayName, bankName, accountNumberLast4} were originally tracked but not in current registry; can be added as a mechanical follow-up PR.
- **Readiness % bump: 85% → 87%.** Confidentiality TSC was the largest "Missing/critically weak" row in the honest summary. 14-column rollout + extension architecture on main warrants a 2pp bump.

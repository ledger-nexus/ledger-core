...
| Role-based access control? | 4-role hierarchy (`OWNER > ADMIN > MEMBER > VIEWER`) × 16 named permissions per tenant. Single source of truth: `src/lib/auth/policy.ts`. Every Server Action calls `requirePermission(...)`. | access-control v2.0 + `src/lib/auth/policy.ts` |

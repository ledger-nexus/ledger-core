...
import { getTranslationRate, FxRateNotSeededError } from "@/lib/accounting/fx";
...
describe("getTranslationRate", () => {
  ...
  it("EXCLUDED category returns rate=1 without a DB lookup", async () => {
    ...
    // would throw FxRateNotSeededError. The pass proves the
  ...
  it("CURRENT_RATE throws FxRateNotSeededError when no rate exists", async () => {

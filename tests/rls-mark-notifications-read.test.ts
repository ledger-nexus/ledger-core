...
import { withTenantContext, currentTenantId } from "../src/lib/tenant-context";
...
describe("RLS Phase 2b — markRead works inside withTenantContext", () => {
  it("markRead invoked with tx client successfully marks notifications", async () => {
    ...
    const { markedCount } = await withTenantContext(prisma, TEST_TENANT_ID, async (tx) => {
  ...
  it("currentTenantId returns the tenantId inside the transaction (proves plumbing)", async () => {
    let observedGuc: string | null = null;
    await withTenantContext(prisma, TEST_TENANT_ID, async (tx) => {

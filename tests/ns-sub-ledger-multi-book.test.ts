...
async function cleanup(): Promise<void> {
  ...
  // Neutralize any OTHER suite's entity-scoped rows still holding these
  // NS account lineage ids. The importer's lineage dedupe is tenant-
  // global, so a foreign entity's row (e.g. NS_DEMO's NS1200) would
  // starve this suite's account creation — the cross-suite lineage
  // starvation the FX suites neutralize the same way. FK-safe: only
  // lineage fields are nulled; the rows stay for their own suite's
  // adopt-before-create to re-attach.

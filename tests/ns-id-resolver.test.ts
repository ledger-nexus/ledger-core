...
beforeAll(async () => {
  ...
  const tenantA = await prisma.tenant.create({
    data: { slug: `nsres-${SUFFIX}-a`, name: "Resolver A", ownerUserId: owner.id },
  });
  const tenantB = await prisma.tenant.create({
    data: { slug: `nsres-${SUFFIX}-b`, name: "Resolver B", ownerUserId: owner.id },
  });
...
describe("v0.9 NS SuiteAnalytics Phase 2: resolveNsAccount", () => {
  it("returns { accountCode, accountId } for a seeded NS account internalid", async () => {
    const result = await resolveNsAccount(prisma, { nsInternalid: "100" });
    expect(result).not.toBeNull();
  ...
  it("returns null for an unknown NS account internalid", async () => {
    const result = await resolveNsAccount(prisma, { nsInternalid: "999" });
    expect(result).toBeNull();
  });
});

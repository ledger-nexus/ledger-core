...
// CC6.8: subject ids arrive from the client — validate the shape before
// any query touches them.
const SubjectId = z.string().uuid();
import {
...
export async function exportUserDataAction(
  ...
  try {
    const parsedId = SubjectId.safeParse(subjectUserId);
    if (!parsedId.success) {
  ...
  try {
    ...
    await logAuditEvent({
      ...
      metadata: {
        ...
        journalEntryNotesRedacted: summary.journalEntryNotesRedacted,
      },
    });

    return {
      ok: true,
      message: `Erased PII for user ${subjectUserId.slice(0, 8)}… (${summary.emailDeliveriesRedacted} email deliveries and ${summary.journalEntryNotesRedacted} note-author snapshots also redacted). The user can no longer sign in.`,
      payload: summary,
    };
  } catch (e) {
    return mapError(e);

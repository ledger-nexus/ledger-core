...
//   periodStart: REJECTED with 400 — ASC 830 translation dispositioned
//   unmerged (PROJECT_STATUS v1.27); remeasurement method only.
...
export async function GET(req: NextRequest): Promise<NextResponse> {
  ...
  if (periodStart !== null) {
    // ASC 830 current-rate translation was dispositioned unmerged
    // (PROJECT_STATUS v1.27): main is on the remeasurement method and
    // getConsolidatedTrialBalance has no periodStart/translation
    // surface. Reject explicitly rather than silently ignoring the
    // param a BI tool thinks it is using.
    return NextResponse.json(
      {
        error:
          "periodStart (ASC 830 translation) is not supported: this " +
  ...
  return NextResponse.json({
    ...
    // Translation/CTA keys intentionally null: dispositioned unmerged
    // (v1.27); kept as keys so NS-shape-strict BI adapters don't crash.

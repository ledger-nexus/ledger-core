...
export interface ErasureSummary {
  ...
  /** JE-note author snapshots redacted (authorEmail is a point-in-time
   *  copy of the subject's address — the note BODY stays, per the
   *  financial-record retention exemption). */
  journalEntryNotesRedacted: number;
}
...
export async function eraseUserPii(
  ...
  const result = await prisma.$transaction(async (tx) => {
    ...
    //    Equality on the encrypted toEmail column resolves via the
    //    extension's search-hash rewrite; the redacted value re-encrypts
    //    + re-hashes on write.
    ...
    // 3. Redact the authorEmail snapshot on the subject's JE notes.
    //    (Post-#46 column.) authorEmail is encrypted WITHOUT a search
    //    hash — deliberately, see the registry — so it cannot be
    //    filtered by value; authorUserId is the reliable key. The note
    //    BODY stays: it annotates a financial record and falls under
    //    the Art. 17(3) retention exemption, but the identity snapshot
    //    beside it does not.
    const notesResult = await tx.journalEntryNote.updateMany({
      where: { authorUserId: userId },
      data: { authorEmail: redactedEmail },
    });
    ...
    return {
      ...
      journalEntryNotesRedacted: notesResult.count,
    };
  });

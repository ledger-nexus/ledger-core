...
- **The audit_log trigger blocks UPDATE only — DELETE-block is deficiency #14 (Open)** — the first draft also blocked DELETE; the PR #232 CI run failed 21 tests across 12 files because the suite isolates audit assertions by deleting its own audit rows (afterAll hooks + delete-then-assert), which also proves the original trigger never blocked DELETE. Compensating controls noted in the deficiency log. To close #14: rework test audit-row isolation first, then add the `no_delete` trigger to the mirror file.

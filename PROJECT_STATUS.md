...
- **Sentry shim shipped portfolio-wide** (closes deficiency #5 at the code layer; 4 companion ports + ledger-core upstream). First Medium-severity deficiency closed by genuine new code this session — every prior closure was either Low (#25, #26) or verification-of-pre-existing-state (#13, #18). Integrations port pins OAuth-token-leak prevention (Critical incident class).

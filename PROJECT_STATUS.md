...
**Current state:** **v0.9 NS arc fully closed (Phases 1 → 5 + SuiteAnalytics + Arc 6 burndown + 34th adversarial pass + Arc 8 v1.2+ polish closure).** Today's sprint shipped 49 PRs across 6 architectural arcs + 1 self-review pass + 1 polish-arc closure. Bi-directional NS substrate operational: ledger-core ingests NS OneWorld data (multi-sub + multi-book + ASC 830 FX + per-book sub-ledgers) AND serves NS-canonical SuiteAnalytics-shaped reports back to BI tools (bearer-auth + NS-internalid resolution + TB/IS/BS/Saved-Search/Consolidated-TB + JSON + CSV). Every documented deferral closed in code — the v1.2+ ergonomics-and-polish list that had been carrying items since v0.6 is now empty (one item closed as WON'T DO with ASC 830 evidence: HISTORICAL line-walking). 162+ tests across 28+ files. 0 deferrals remain. See [MERGE_ORDER_2026-06-08.md](./MERGE_ORDER_2026-06-08.md) for the full 49-PR train.
...
- [x] ~~Keyboard shortcut for "+ Add line" (Tab from last cell)~~ shipped 2026-06-08. Tab on the last row's Amount field appends a new line with the same Dr/Cr side and focuses its Side select. Implementation in `src/app/journal-entries/new/new-entry-form.tsx` via a `pendingFocusLastSide` ref + `useEffect` keyed on `lines.length` that runs a `document.querySelectorAll<HTMLSelectElement>('select[data-line-side]')` scoped to the form ref.
...
- [x] ~~AR / AP aging with sortable columns~~ shipped 2026-06-08. Both `/reports/ar-aging` and `/reports/ap-aging` now carry sortable column headers via `?sortKey=…&sortDir=asc|desc` URL params. SSR-only (no client JS, no hydration), deep-linkable, browser back/forward respects sort state. In-memory sort handles derived columns (`daysOverdue`) naturally without a SQL CASE expression. Default ordering preserved: `due asc` (the original behavior). 8 sortable keys per page: reference, customer/vendor, opened, due, daysOverdue, status, original, balance.
- [x] ~~**HISTORICAL category line-walking**~~ closed 2026-06-08 as **WON'T DO** — the pass-through is ASC 830-correct, not a polish TODO. ASC 830-10-45-9 says non-monetary items measured at historical cost are translated at the rate IN EFFECT WHEN the item was recognized. The GL already stores `line.debit` in book reporting currency at posting-time rate (per `postJournalEntry`'s `fxRate` parameter). So pass-through PRESERVES the historical rate that ASC 830 mandates. Per-line walking would yield the same number under the current schema. New `tests/fx-consolidation-historical.test.ts` proves the dimensional analysis with a worked example: GBP equity contribution at posting-rate 1.20 stays at 1,200 USD; the CR cash counterpart remeasures to 1,560 USD at current 1.30; the 360 USD CTA plug is exactly what ASC 830 wants on the equity section. Comment in `consolidation.ts:314` rewritten to document the rationale (previous "TODO" was misleading carry-over from a prior author's misunderstanding).
